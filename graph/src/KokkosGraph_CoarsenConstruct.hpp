 ...
 public:
    ...
    if (handle.b == Hashmap || is_host_space) {
      ...
      do {
        ...
        if (!is_host_space && max_entries >= 128) {
          Kokkos::parallel_reduce("hashmap time", team_policy_t(old_remaining_count, 1, team_policy_t::vector_length_max()), hashmapAccumulator,
                                  remaining_count);
        } else {
          if (use_dyn) {
      ...
      while (remaining_count > 0) {
        ...
        ordinal_t old_remaining_count = remaining_count;
        if (!is_host_space && max_entries >= 128) {
          Kokkos::parallel_reduce("hashmap time", dyn_team_policy_t(old_remaining_count, 1, dyn_team_policy_t::vector_length_max()), hashmapAccumulator,
                                  remaining_count);
        } else {
  ...
  static coarse_level_triple build_nonskew(coarsen_handle& handle, const matrix_t g, const matrix_t vcmap,
    ...
    if (is_host_space) {
      ...
      vectorLength = (vectorLength < team_policy_t::vector_length_max() ? vectorLength : team_policy_t::vector_length_max());
