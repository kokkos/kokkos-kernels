...
KOKKOS_INLINE_FUNCTION int SerialGbtrsInternal<Trans::Transpose, Algo::Gbtrs::Unblocked>::invoke(
  ...
  if (lnoti) {
    for (int j = n - 2; j >= 0; --j) {
      ...
      KokkosBlas::Impl::SerialGemvInternal<Algo::Gemv::Unblocked>::invoke(1, a.extent(0), -1.0, a.data(), a.stride(0),
                                                                          a.stride(0), x.data(), x.stride(0), 1.0,
                                                                          y.data(), y.stride(0));
  ...
  if (lnoti) {
    for (int j = n - 2; j >= 0; --j) {
      ...
      KokkosBlas::Impl::SerialGemvInternal<Algo::Gemv::Unblocked>::invoke(
          KokkosBlas::Impl::OpConj(), 1, a.extent(0), -1.0, a.data(), a.stride(0), a.stride(0), x.data(),
          x.stride(0), 1.0, y.data(), y.stride(0));
      SerialLacgv::invoke(b_j);
