...
struct Functor_TestBatchedTeamAxpy {
  ...
  Functor_TestBatchedTeamAxpy(const alphaViewType &alpha, const ViewType &X, const ViewType &Y, const int N_team)
...
void impl_test_batched_axpy(const int N, const int BlkSize, const int N_team) {
  ...
  Functor_TestBatchedTeamAxpy<DeviceType, ViewType, alphaViewType>(alpha, X1, Y1, N_team).run();
