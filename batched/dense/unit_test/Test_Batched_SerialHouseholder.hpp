...
void test_Householder_analytic_real() {
  ...
  vec_h(0) = 4; vec_h(1) = 2; vec_h(2) = 2; vec_h(3) = 1;
  ...
  Test::EXPECT_NEAR_KK_REL(tau_h(0), 10.0 / 18.0, tol);
...
void test_Householder_analytic_cplx() {
  ...
  vec_type vec("vector to reflect", 4), tau("tau", 1);
  auto vec_h = Kokkos::create_mirror(vec);
  vec_h(0) = Kokkos::complex(4., 3.);
  vec_h(1) = Kokkos::complex(2., 5.);
  vec_h(2) = Kokkos::complex(2., 5.);
  vec_h(3) = Kokkos::complex(1., 4.);
  Kokkos::deep_copy(vec, vec_h);
