...
struct Functor_TestBatchedTeamGMRES {
  ...
  Functor_TestBatchedTeamGMRES(const ValuesViewType &D, const IntView &r, const IntView &c, const VectorViewType &X,
  ...
  KOKKOS_INLINE_FUNCTION void operator()(const MemberType &member) const {
    ...
    KokkosBatched::TeamGMRES<MemberType>::template invoke<Operator, VectorViewType>(member, A, b, x, P, _handle);
  }
...
int test_batched_team_GMRES() {
#if defined(KOKKOSKERNELS_INST_LAYOUTLEFT)
  {
    ...
    for (int i = 3; i < 10; ++i) {
      Test::TeamGMRES::impl_test_batched_GMRES<DeviceType, ViewType, IntView, VectorViewType>(1024, i, 2);
    }
  }
#endif
