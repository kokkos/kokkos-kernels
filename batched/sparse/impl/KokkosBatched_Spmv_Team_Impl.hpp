...
struct TeamSpmv<MemberType, Trans::NoTranspose> {
  ...
  KOKKOS_INLINE_FUNCTION static int invoke(const MemberType& member, const alphaViewType& alpha,
    ...
    return TeamSpmvInternal::template invoke<
        ...
        member, X.extent(0), X.extent(1), alpha.data(), alpha.stride(0), values.data(), values.stride(0),
        values.stride(1), row_ptr.data(), row_ptr.stride(0), colIndices.data(), colIndices.stride(0), X.data(),
        X.stride(0), X.stride(1), beta.data(), beta.stride(0), Y.data(), Y.stride(0), Y.stride(1));
  }
...
#endif
    if (values.extent(0) == 1) {
      return KokkosSparse::Experimental::team_spmv<MemberType>(member, alpha, Kokkos::subview(values, 0, Kokkos::ALL),
                                                               row_ptr, colIndices, Kokkos::subview(X, 0, Kokkos::ALL),
                                                               beta, Kokkos::subview(Y, 0, Kokkos::ALL), dobeta);
    }
