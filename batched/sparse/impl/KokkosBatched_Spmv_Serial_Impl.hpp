...
struct SerialSpmv<Trans::NoTranspose> {
  ...
  KOKKOS_INLINE_FUNCTION static int invoke(const alphaViewType& alpha, const ValuesViewType& values,
    ...
    return SerialSpmvInternal::template invoke<
        ...
        row_ptr.data(), row_ptr.stride(0), colIndices.data(), colIndices.stride(0), X.data(), X.stride(0),
        X.stride(1), beta.data(), beta.stride(0), Y.data(), Y.stride(0), Y.stride(1));
  }
  ...
  KOKKOS_INLINE_FUNCTION static int invoke(
    ...
    return SerialSpmvInternal::template invoke<
        ...
        X.extent(0), X.extent(1), alpha, values.data(), values.stride(0), values.stride(1), row_ptr.data(),
