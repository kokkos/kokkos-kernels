...
class SyrTester {
 ...
 private:
  ...
  typename std::enable_if<std::is_same<T, Kokkos::complex<float>>::value ||
                          ...
                          void>::type
  populateAnalyticalValues(T& alpha, _HostViewTypeX& h_x, _HostViewTypeA& h_A,
...
template <class ScalarX, class tLayoutX, class ScalarA, class tLayoutA,
          class Device>
void SyrTester<ScalarX, tLayoutX, ScalarA, tLayoutA, Device>::test(
...
typename std::enable_if<std::is_same<T, Kokkos::complex<float>>::value ||
                            std::is_same<T, Kokkos::complex<double>>::value,
                        void>::type
SyrTester<ScalarX, tLayoutX, ScalarA, tLayoutA,
...
SyrTester<ScalarX, tLayoutX, ScalarA, tLayoutA,
  ...
  for (int i = 0; i < _M; ++i) {
    ...
    for (int j = 0; j < _N; ++j) {
      _AuxType auxJ =
          this->shrinkAngleToZeroTwoPiRange(static_cast<_AuxType>(j));
...
typename std::enable_if<std::is_same<T, Kokkos::complex<float>>::value ||
                            std::is_same<T, Kokkos::complex<double>>::value,
                        void>::type
SyrTester<ScalarX, tLayoutX, ScalarA, tLayoutA, Device>::populateVanillaValues(
    const T& alpha, const _HostViewTypeX& h_x, const _HostViewTypeA& h_A,
    _ViewTypeExpected& h_vanilla) {
...
template <class ScalarX, class tLayoutX, class ScalarA, class tLayoutA,
          class Device>
template <class T>
T SyrTester<ScalarX, tLayoutX, ScalarA, tLayoutA,
...
void SyrTester<ScalarX, tLayoutX, ScalarA, tLayoutA, Device>::
  ...
  EXPECT_EQ(gotStdException, _kkSyrShouldThrowException)
      << "Failed test, '" << situation << "': kk syr() should"
...
int test_syr(const std::string& caseName) {
  ...
  if (true) {
    Test::SyrTester<ScalarX, Kokkos::LayoutLeft, ScalarA, Kokkos::LayoutRight,
                    Device>
        tester;
    tester.test(1024, 0);
  }
...
TEST_F(TestCategory, syr_complex_float) {
  ...
  test_syr<Kokkos::complex<float>, Kokkos::complex<float>, TestExecSpace>(
      "test case syr_complex_float");
  Kokkos::Profiling::popRegion();
}
#endif
...
TEST_F(TestCategory, syr_complex_double) {
  ...
  test_syr<Kokkos::complex<double>, Kokkos::complex<double>, TestExecSpace>(
      "test case syr_complex_double");
  Kokkos::Profiling::popRegion();
}
#endif
