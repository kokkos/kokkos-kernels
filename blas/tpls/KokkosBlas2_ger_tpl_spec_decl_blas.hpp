...
#define KOKKOSBLAS2_DGER_BLAS(LAYOUT, MEM_SPACE, ETI_SPEC_AVAIL)           \
  ...
  struct GER<                                                              \
    ...
    static void ger(const typename AViewType::execution_space& /* space */ \
      ...
      if (A_is_ll) {                                                       \
        HostBlas<SCALAR>::ger(M, N, alpha, X.data(), one, Y.data(), one,   \
                              A.data(), LDA);                              \
      } else {                                                             \
        HostBlas<SCALAR>::ger(M, N, alpha, Y.data(), one, X.data(), one,   \
                              A.data(), LDA);                              \
      }                                                                    \
...
#define KOKKOSBLAS2_ZGER_BLAS(LAYOUT, MEM_SPACE, ETI_SPEC_AVAIL)            \
  ...
  struct GER<Kokkos::View<const Kokkos::complex<double>*, LAYOUT,           \
    ...
    static void ger(const typename AViewType::execution_space& /* space */  \
      ...
      if (A_is_ll) {                                                        \
        if (justTranspose) {                                                \
          HostBlas<std::complex<double>>::geru(                             \
              M, N, alpha_val,                                              \
          ...
          HostBlas<std::complex<double>>::gerc(                             \
              M, N, alpha_val,                                              \
              reinterpret_cast<const std::complex<double>*>(X.data()), one, \
              reinterpret_cast<const std::complex<double>*>(Y.data()), one, \
              reinterpret_cast<std::complex<double>*>(A.data()), LDA);      \
    ...
    static void ger(const typename AViewType::execution_space& /* space */ \
      ...
      if (A_is_ll) {                                                       \
        if (justTranspose) {                                               \
          ...
          HostBlas<std::complex<float>>::gerc(                             \
              M, N, alpha_val,                                             \
              reinterpret_cast<const std::complex<float>*>(X.data()), one, \
              reinterpret_cast<const std::complex<float>*>(Y.data()), one, \
              reinterpret_cast<std::complex<float>*>(A.data()), LDA);      \
