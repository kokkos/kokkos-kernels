...
#define KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_CUBLAS(LAYOUT, KOKKOS_TYPE, TPL_TYPE,   \
                                              EXECSPACE, MEMSPACE, TPL_NRM2,   \
                                              ETI_SPEC_AVAIL)                  \
  template <>                                                                  \
  struct Nrm2<EXECSPACE,                                                       \
              Kokkos::View<Kokkos::ArithTraits<KOKKOS_TYPE>::mag_type, LAYOUT, \
                           Kokkos::HostSpace,                                  \
                           Kokkos::MemoryTraits<Kokkos::Unmanaged> >,          \
              Kokkos::View<const KOKKOS_TYPE*, LAYOUT,                         \
                           Kokkos::Device<EXECSPACE, MEMSPACE>,                \
                           Kokkos::MemoryTraits<Kokkos::Unmanaged> >,          \
              1, true, ETI_SPEC_AVAIL> {                                       \
    using RT        = Kokkos::ArithTraits<KOKKOS_TYPE>::mag_type;              \
    using RV        = Kokkos::View<RT, LAYOUT, Kokkos::HostSpace,              \
                            Kokkos::MemoryTraits<Kokkos::Unmanaged> >;  \
    using XV        = Kokkos::View<const KOKKOS_TYPE*, LAYOUT,                 \
                            Kokkos::Device<EXECSPACE, MEMSPACE>,        \
                            Kokkos::MemoryTraits<Kokkos::Unmanaged> >;  \
    ...
    static void nrm2(const EXECSPACE& space, RV& R, const XV& X,               \
                     const bool& take_sqrt) {                                  \
      Kokkos::Profiling::pushRegion("KokkosBlas::nrm2[TPL_CUBLAS," +           \
                                    Kokkos::ArithTraits<KOKKOS_TYPE>::name() + \
                                    "]");                                      \
      const size_type numElems = X.extent(0);                                  \
      if (numElems <=                                                          \
        ...
        KOKKOS_CUBLAS_SAFE_CALL_IMPL(                                          \
            TPL_NRM2(s.handle, N, reinterpret_cast<const TPL_TYPE*>(X.data()), \
                     1, &R()));                                                \
...
#define KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_CUBLAS_EXT(ETI_SPEC_AVAIL)           \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_CUBLAS(Kokkos::LayoutLeft, float, float,   \
                                        Kokkos::Cuda, Kokkos::CudaSpace,    \
                                        cublasSnrm2, ETI_SPEC_AVAIL)        \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_CUBLAS(Kokkos::LayoutLeft, double, double, \
                                        Kokkos::Cuda, Kokkos::CudaSpace,    \
                                        cublasDnrm2, ETI_SPEC_AVAIL)        \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_CUBLAS(                                    \
      Kokkos::LayoutLeft, Kokkos::complex<float>, cuComplex, Kokkos::Cuda,  \
      Kokkos::CudaSpace, cublasScnrm2, ETI_SPEC_AVAIL)                      \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_CUBLAS(                                    \
      Kokkos::LayoutLeft, Kokkos::complex<double>, cuDoubleComplex,         \
      Kokkos::Cuda, Kokkos::CudaSpace, cublasDznrm2, ETI_SPEC_AVAIL)
...
#define KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ROCBLAS(LAYOUT, KOKKOS_TYPE, TPL_TYPE,  \
                                               EXECSPACE, MEMSPACE, TPL_NRM2,  \
                                               ETI_SPEC_AVAIL)                 \
  template <>                                                                  \
  struct Nrm2<EXECSPACE,                                                       \
    ...
    static void nrm2(const EXECSPACE& space, RV& R, const XV& X,               \
      ...
      const size_type numElems = X.extent(0);                                  \
      if (numElems <=                                                          \
          static_cast<size_type>(std::numeric_limits<rocblas_int>::max())) {   \
        nrm2_print_specialization<RV, XV>();                                   \
...
#define KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ROCBLAS_EXT(ETI_SPEC_AVAIL)           \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ROCBLAS(Kokkos::LayoutLeft, float, float,   \
                                         Kokkos::HIP, Kokkos::HIPSpace,      \
                                         rocblas_snrm2, ETI_SPEC_AVAIL)      \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ROCBLAS(Kokkos::LayoutLeft, double, double, \
                                         Kokkos::HIP, Kokkos::HIPSpace,      \
                                         rocblas_dnrm2, ETI_SPEC_AVAIL)      \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ROCBLAS(                                    \
      Kokkos::LayoutLeft, Kokkos::complex<float>, rocblas_float_complex,     \
      Kokkos::HIP, Kokkos::HIPSpace, rocblas_scnrm2, ETI_SPEC_AVAIL)         \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ROCBLAS(                                    \
      Kokkos::LayoutLeft, Kokkos::complex<double>, rocblas_double_complex,   \
...
#define KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ONEMKL(LAYOUT, KOKKOS_TYPE, TPL_TYPE,   \
                                              EXECSPACE, MEMSPACE, TPL_NRM2,   \
                                              ETI_SPEC_AVAIL)                  \
  template <>                                                                  \
  struct Nrm2<EXECSPACE,                                                       \
    ...
    static void nrm2(const EXECSPACE& space, RV& R, const XV& X,               \
                     const bool& take_sqrt) {                                  \
      Kokkos::Profiling::pushRegion("KokkosBlas::nrm2[TPL_ONEMKL," +           \
                                    Kokkos::ArithTraits<KOKKOS_TYPE>::name() + \
                                    "]");                                      \
      const size_type numElems = X.extent(0);                                  \
      if (numElems <=                                                          \
          static_cast<size_type>(std::numeric_limits<std::int64_t>::max())) {  \
        nrm2_print_specialization<RV, XV>();                                   \
        const std::int64_t N = static_cast<std::int64_t>(numElems);            \
        TPL_NRM2(space.sycl_queue(), N,                                        \
                 reinterpret_cast<const TPL_TYPE*>(X.data()), 1, &R());        \
        if (!take_sqrt) R() = R() * R();                                       \
      } else {                                                                 \
        Nrm2<EXECSPACE, RV, XV, 1, false, ETI_SPEC_AVAIL>::nrm2(space, R, X,   \
                                                                take_sqrt);    \
...
#define KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ONEMKL_EXT(ETI_SPEC_AVAIL)           \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ONEMKL(                                    \
      Kokkos::LayoutLeft, float, float, Kokkos::Experimental::SYCL,         \
      Kokkos::Experimental::SYCLDeviceUSMSpace,                             \
      oneapi::mkl::blas::row_major::nrm2, ETI_SPEC_AVAIL)                   \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ONEMKL(                                    \
      Kokkos::LayoutLeft, double, double, Kokkos::Experimental::SYCL,       \
      Kokkos::Experimental::SYCLDeviceUSMSpace,                             \
      oneapi::mkl::blas::row_major::nrm2, ETI_SPEC_AVAIL)                   \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ONEMKL(                                    \
      Kokkos::LayoutLeft, Kokkos::complex<float>, std::complex<float>,      \
      Kokkos::Experimental::SYCL, Kokkos::Experimental::SYCLDeviceUSMSpace, \
      oneapi::mkl::blas::row_major::nrm2, ETI_SPEC_AVAIL)                   \
  KOKKOSBLAS1_NRM2_TPL_SPEC_DECL_ONEMKL(                                    \
      Kokkos::LayoutLeft, Kokkos::complex<double>, std::complex<double>,    \
      Kokkos::Experimental::SYCL, Kokkos::Experimental::SYCLDeviceUSMSpace, \
      oneapi::mkl::blas::row_major::nrm2, ETI_SPEC_AVAIL)
