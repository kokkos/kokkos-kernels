...
#define KOKKOSBLAS2_DSYR2_ROCBLAS(LAYOUT, EXEC_SPACE, MEM_SPACE,             \
                                  ETI_SPEC_AVAIL)                            \
  template <>                                                                \
  struct SYR2<                                                               \
      EXEC_SPACE,                                                            \
    ...
    static void syr2(const typename AViewType::execution_space& space,       \
      ...
      if (A_is_ll) {                                                         \
        ...
        KOKKOS_ROCBLAS_SAFE_CALL_IMPL(                                       \
            rocblas_dsyr2(s.handle, fillMode, N, &alpha, X.data(), one,      \
                          Y.data(), one, A.data(), LDA));                    \
        KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_set_stream(s.handle, NULL));   \
      } else {                                                               \
        /* rocblas_dsyr2() + ~A_ll => call kokkos-kernels' implementation */ \
        SYR2<EXEC_SPACE, XViewType, YViewType, AViewType, false,             \
...
#define KOKKOSBLAS2_SSYR2_ROCBLAS(LAYOUT, EXEC_SPACE, MEM_SPACE,             \
                                  ETI_SPEC_AVAIL)                            \
  template <>                                                                \
  struct SYR2<                                                               \
      EXEC_SPACE,                                                            \
    ...
    static void syr2(const typename AViewType::execution_space& space,       \
      ...
      if (A_is_ll) {                                                         \
        ...
        KOKKOS_ROCBLAS_SAFE_CALL_IMPL(                                       \
            rocblas_set_stream(s.handle, space.hip_stream()));               \
        KOKKOS_ROCBLAS_SAFE_CALL_IMPL(                                       \
            rocblas_ssyr2(s.handle, fillMode, N, &alpha, X.data(), one,      \
                          Y.data(), one, A.data(), LDA));                    \
        KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_set_stream(s.handle, NULL));   \
      } else {                                                               \
...
#define KOKKOSBLAS2_ZSYR2_ROCBLAS(LAYOUT, EXEC_SPACE, MEM_SPACE,               \
                                  ETI_SPEC_AVAIL)                              \
  template <>                                                                  \
  struct SYR2<EXEC_SPACE,                                                      \
    ...
    static void syr2(const typename AViewType::execution_space& space,         \
      ...
      Kokkos::Profiling::pushRegion(                                           \
          "KokkosBlas::syr2[TPL_ROCBLAS,complex<double>]");                    \
      KOKKOSBLAS2_SYR2_ROCBLAS_DETERMINE_ARGS(LAYOUT, uplo[0]);                \
      ...
      if (justTranspose) {                                                     \
        if (A_is_ll) {                                                         \
          ...
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(                                       \
              rocblas_set_stream(s.handle, space.hip_stream()));               \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_zsyr2(                         \
              s.handle, fillMode, N,                                           \
        ...
        if (A_is_ll && (alpha.imag() == 0.)) {                                 \
          ...
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(                                       \
              rocblas_set_stream(s.handle, space.hip_stream()));               \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_zher2(                         \
              s.handle, fillMode, N,                                           \
              reinterpret_cast<const rocblas_double_complex*>(&alpha),         \
              reinterpret_cast<const rocblas_double_complex*>(X.data()), one,  \
              reinterpret_cast<const rocblas_double_complex*>(Y.data()), one,  \
...
#define KOKKOSBLAS2_CSYR2_ROCBLAS(LAYOUT, EXEC_SPACE, MEM_SPACE,               \
                                  ETI_SPEC_AVAIL)                              \
  template <>                                                                  \
  struct SYR2<EXEC_SPACE,                                                      \
    ...
    static void syr2(const typename AViewType::execution_space& space,         \
                     ...
                     const AViewType& A) {                                     \
      Kokkos::Profiling::pushRegion(                                           \
          "KokkosBlas::syr2[TPL_ROCBLAS,complex<float>]");                     \
      KOKKOSBLAS2_SYR2_ROCBLAS_DETERMINE_ARGS(LAYOUT, uplo[0]);                \
      bool justTranspose = (trans[0] == 'T') || (trans[0] == 't');             \
      if (justTranspose) {                                                     \
        if (A_is_ll) {                                                         \
          ...
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(                                       \
              rocblas_set_stream(s.handle, space.hip_stream()));               \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_csyr2(                         \
              s.handle, fillMode, N,                                           \
        ...
        if (A_is_ll && (alpha.imag() == 0.)) {                                 \
          ...
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(                                       \
              rocblas_set_stream(s.handle, space.hip_stream()));               \
          KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocblas_cher2(                         \
              s.handle, fillMode, N,                                           \
              reinterpret_cast<const rocblas_float_complex*>(&alpha),          \
              reinterpret_cast<const rocblas_float_complex*>(X.data()), one,   \
              reinterpret_cast<const rocblas_float_complex*>(Y.data()), one,   \
...
KOKKOSBLAS2_DSYR2_ROCBLAS(Kokkos::LayoutLeft, Kokkos::HIP, Kokkos::HIPSpace,
                          true)
KOKKOSBLAS2_DSYR2_ROCBLAS(Kokkos::LayoutLeft, Kokkos::HIP, Kokkos::HIPSpace,
                          false)
KOKKOSBLAS2_DSYR2_ROCBLAS(Kokkos::LayoutRight, Kokkos::HIP, Kokkos::HIPSpace,
                          true)
KOKKOSBLAS2_DSYR2_ROCBLAS(Kokkos::LayoutRight, Kokkos::HIP, Kokkos::HIPSpace,
                          false)

KOKKOSBLAS2_SSYR2_ROCBLAS(Kokkos::LayoutLeft, Kokkos::HIP, Kokkos::HIPSpace,
                          true)
KOKKOSBLAS2_SSYR2_ROCBLAS(Kokkos::LayoutLeft, Kokkos::HIP, Kokkos::HIPSpace,
                          false)
KOKKOSBLAS2_SSYR2_ROCBLAS(Kokkos::LayoutRight, Kokkos::HIP, Kokkos::HIPSpace,
                          true)
KOKKOSBLAS2_SSYR2_ROCBLAS(Kokkos::LayoutRight, Kokkos::HIP, Kokkos::HIPSpace,
                          false)

KOKKOSBLAS2_ZSYR2_ROCBLAS(Kokkos::LayoutLeft, Kokkos::HIP, Kokkos::HIPSpace,
                          true)
KOKKOSBLAS2_ZSYR2_ROCBLAS(Kokkos::LayoutLeft, Kokkos::HIP, Kokkos::HIPSpace,
                          false)
KOKKOSBLAS2_ZSYR2_ROCBLAS(Kokkos::LayoutRight, Kokkos::HIP, Kokkos::HIPSpace,
                          true)
KOKKOSBLAS2_ZSYR2_ROCBLAS(Kokkos::LayoutRight, Kokkos::HIP, Kokkos::HIPSpace,
                          false)

KOKKOSBLAS2_CSYR2_ROCBLAS(Kokkos::LayoutLeft, Kokkos::HIP, Kokkos::HIPSpace,
                          true)
KOKKOSBLAS2_CSYR2_ROCBLAS(Kokkos::LayoutLeft, Kokkos::HIP, Kokkos::HIPSpace,
                          false)
KOKKOSBLAS2_CSYR2_ROCBLAS(Kokkos::LayoutRight, Kokkos::HIP, Kokkos::HIPSpace,
                          true)
KOKKOSBLAS2_CSYR2_ROCBLAS(Kokkos::LayoutRight, Kokkos::HIP, Kokkos::HIPSpace,
                          false)
