...
#define KOKKOSBLAS2_DSYR_CUBLAS(LAYOUT, EXEC_SPACE, ETI_SPEC_AVAIL)                                                 \
  template <>                                                                                                       \
...
#define KOKKOSBLAS2_SSYR_CUBLAS(LAYOUT, EXEC_SPACE, ETI_SPEC_AVAIL)                                                \
  template <>                                                                                                      \
...
#define KOKKOSBLAS2_ZSYR_CUBLAS(LAYOUT, EXEC_SPACE, ETI_SPEC_AVAIL)                                                \
  template <>                                                                                                      \
  struct SYR<                                                                                                      \
      EXEC_SPACE,                                                                                                  \
    ...
    static void syr(const typename AViewType::execution_space& space, const char trans[], const char uplo[],       \
      ...
      if (justTranspose) {                                                                                         \
        if (A_is_ll) {                                                                                             \
          KokkosBlas::Impl::CudaBlasSingleton& s = KokkosBlas::Impl::CudaBlasSingleton::singleton();               \
          KOKKOSBLAS_IMPL_CUBLAS_SAFE_CALL(cublasSetStream(s.handle, space.cuda_stream()));                        \
          KOKKOSBLAS_IMPL_CUBLAS_SAFE_CALL(cublasZsyr(s.handle, fillMode, N,                                       \
        ...
        if (A_is_ll && (alpha.imag() == 0.)) {                                                                     \
          ...
          KokkosBlas::Impl::CudaBlasSingleton& s = KokkosBlas::Impl::CudaBlasSingleton::singleton();               \
          KOKKOSBLAS_IMPL_CUBLAS_SAFE_CALL(cublasSetStream(s.handle, space.cuda_stream()));                        \
          KOKKOSBLAS_IMPL_CUBLAS_SAFE_CALL(cublasZher(s.handle, fillMode, N, &alpha_val,                           \
                                                      reinterpret_cast<const cuDoubleComplex*>(X.data()), one,     \
                                                      reinterpret_cast<cuDoubleComplex*>(A.data()), LDA));         \
          KOKKOSBLAS_IMPL_CUBLAS_SAFE_CALL(cublasSetStream(s.handle, NULL));                                       \
        } else {                                                                                                   \
...
#define KOKKOSBLAS2_CSYR_CUBLAS(LAYOUT, EXEC_SPACE, ETI_SPEC_AVAIL)                                               \
  template <>                                                                                                     \
  struct SYR<                                                                                                     \
      EXEC_SPACE,                                                                                                 \
    ...
    static void syr(const typename AViewType::execution_space& space, const char trans[], const char uplo[],      \
      ...
      if (justTranspose) {                                                                                        \
        if (A_is_ll) {                                                                                            \
          KokkosBlas::Impl::CudaBlasSingleton& s = KokkosBlas::Impl::CudaBlasSingleton::singleton();              \
          KOKKOSBLAS_IMPL_CUBLAS_SAFE_CALL(cublasSetStream(s.handle, space.cuda_stream()));                       \
          KOKKOSBLAS_IMPL_CUBLAS_SAFE_CALL(cublasCsyr(                                                            \
              s.handle, fillMode, N, reinterpret_cast<const cuComplex*>(&alpha),                                  \
              reinterpret_cast<const cuComplex*>(X.data()), one, reinterpret_cast<cuComplex*>(A.data()), LDA));   \
          KOKKOSBLAS_IMPL_CUBLAS_SAFE_CALL(cublasSetStream(s.handle, NULL));                                      \
        } else {                                                                                                  \
        ...
        if (A_is_ll && (alpha.imag() == 0.)) {                                                                    \
          ...
          KokkosBlas::Impl::CudaBlasSingleton& s = KokkosBlas::Impl::CudaBlasSingleton::singleton();              \
          KOKKOSBLAS_IMPL_CUBLAS_SAFE_CALL(cublasSetStream(s.handle, space.cuda_stream()));                       \
          KOKKOSBLAS_IMPL_CUBLAS_SAFE_CALL(cublasCher(s.handle, fillMode, N, &alpha_val,                          \
...
KOKKOSBLAS2_DSYR_CUBLAS(Kokkos::LayoutLeft, Kokkos::Cuda, true)
KOKKOSBLAS2_DSYR_CUBLAS(Kokkos::LayoutLeft, Kokkos::Cuda, false)
KOKKOSBLAS2_DSYR_CUBLAS(Kokkos::LayoutRight, Kokkos::Cuda, true)
KOKKOSBLAS2_DSYR_CUBLAS(Kokkos::LayoutRight, Kokkos::Cuda, false)

KOKKOSBLAS2_SSYR_CUBLAS(Kokkos::LayoutLeft, Kokkos::Cuda, true)
KOKKOSBLAS2_SSYR_CUBLAS(Kokkos::LayoutLeft, Kokkos::Cuda, false)
KOKKOSBLAS2_SSYR_CUBLAS(Kokkos::LayoutRight, Kokkos::Cuda, true)
KOKKOSBLAS2_SSYR_CUBLAS(Kokkos::LayoutRight, Kokkos::Cuda, false)

KOKKOSBLAS2_ZSYR_CUBLAS(Kokkos::LayoutLeft, Kokkos::Cuda, true)
KOKKOSBLAS2_ZSYR_CUBLAS(Kokkos::LayoutLeft, Kokkos::Cuda, false)
KOKKOSBLAS2_ZSYR_CUBLAS(Kokkos::LayoutRight, Kokkos::Cuda, true)
KOKKOSBLAS2_ZSYR_CUBLAS(Kokkos::LayoutRight, Kokkos::Cuda, false)

KOKKOSBLAS2_CSYR_CUBLAS(Kokkos::LayoutLeft, Kokkos::Cuda, true)
KOKKOSBLAS2_CSYR_CUBLAS(Kokkos::LayoutLeft, Kokkos::Cuda, false)
KOKKOSBLAS2_CSYR_CUBLAS(Kokkos::LayoutRight, Kokkos::Cuda, true)
KOKKOSBLAS2_CSYR_CUBLAS(Kokkos::LayoutRight, Kokkos::Cuda, false)
