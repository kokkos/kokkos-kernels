...
struct Nrm2w {
  static void nrm2w(const execution_space& space, const RMV& R, const XMV& X, const XMV& W,
...
struct Nrm2w<execution_space, RV, XMV, 2, false, KOKKOSKERNELS_IMPL_COMPILE_LIBRARY> {
  ...
  static void nrm2w(const execution_space& space, const RV& R, const XMV& X, const XMV& W,
                    const bool& take_sqrt) {
    ...
    if (numCols == 1) {
      ...
      if (numRows < static_cast<size_type>(INT_MAX)) {
        ...
        typedef std::int64_t index_type;
        V_Nrm2w_Invoke<execution_space, decltype(R0), decltype(X0), index_type>(space, R0, X0, W0,
                                                               take_sqrt);
      }
    } else {
      if (numRows < static_cast<size_type>(INT_MAX) &&
