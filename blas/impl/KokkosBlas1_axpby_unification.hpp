...
KOKKOS_INLINE_FUNCTION typename CoeffScalarType<Coeff>::type getCoefficient(const Coeff& c, const size_type column = 0) {
...
template <typename Coeff, typename size_type=int>
KOKKOS_INLINE_FUNCTION typename CoeffScalarType<Coeff>::type getCoefficientSlow(const Coeff& c, const size_type column = 0) {
  if constexpr (Kokkos::is_view_v<Coeff>) {
    if constexpr (isRank1View<Coeff>()) {
      if (c.extent_int(0) == 1)
        return c(0);
