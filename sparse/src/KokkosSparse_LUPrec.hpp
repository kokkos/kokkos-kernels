...
class LUPrec : public KokkosSparse::Experimental::Preconditioner<CRS> {
 ...
 public:
  ...
  void apply_impl(const Kokkos::View<const ScalarType *, DEVICE> &X, const Kokkos::View<ScalarType *, DEVICE> &Y,
                  const char transM[] = "N", ScalarType alpha = karith::one(), ScalarType beta = karith::zero()) const {
  ...
  void apply_impl(const Kokkos::View<const ScalarType *, DEVICE> &X, const Kokkos::View<ScalarType *, DEVICE> &Y,
    ...
    using UView2d  = typename Kokkos::View<ScalarType **, Layout, DEVICE,
                                          Kokkos::MemoryTraits<Kokkos::Unmanaged | Kokkos::RandomAccess> >;
    using UView2dc = typename Kokkos::View<const ScalarType **, Layout, DEVICE,
                                           Kokkos::MemoryTraits<Kokkos::Unmanaged | Kokkos::RandomAccess> >;
    UView2dc X2d(X.data(), X.extent(0), 1);
    UView2d Y2d(Y.data(), Y.extent(0), 1), tmp2d(_tmp.data(), _tmp.extent(0), 1),
