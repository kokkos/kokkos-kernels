...
template <class ExecutionSpace, class KernelHandle,
          ...
          class ain_values_scalar_view_type>
void sptrsv_analysis_cusparse(ExecutionSpace &space,
  ...
  KOKKOS_CUSPARSE_SAFE_CALL(cusparseCreateCsr(
      &(h->matDescr), static_cast<int64_t>(nrows), static_cast<int64_t>(nrows),
      nnz, (void *)row_map.data(), (void *)entries.data(),
  ...
  KOKKOS_CUSPARSE_SAFE_CALL(cusparseCreateDnVec(&(h->vecBDescr_dummy),
  ...
  KOKKOS_CUSPARSE_SAFE_CALL(cusparseCreateDnVec(&(h->vecXDescr_dummy),
                                                static_cast<int64_t>(nrows),
  ...
  if constexpr (std::is_same<scalar_type, double>::value) {
    KOKKOS_CUSPARSE_SAFE_CALL(cusparseDcsrsv2_bufferSize(
    ...
    KOKKOS_CUSPARSE_SAFE_CALL(cusparseScsrsv2_bufferSize(
        h->handle, h->transpose, nrows, nnz, h->descr,
        const_cast<float *>(values.data()), row_map.data(), entries.data(),
        h->info, &pBufferSize));
    ...
    KOKKOS_CUSPARSE_SAFE_CALL(cusparseScsrsv2_analysis(
        h->handle, h->transpose, nrows, nnz, h->descr, values.data(),
        row_map.data(), entries.data(), h->info, h->policy, h->pBuffer));
  } else if constexpr (std::is_same<scalar_type,
                                    Kokkos::complex<double> >::value) {
    KOKKOS_CUSPARSE_SAFE_CALL(cusparseZcsrsv2_bufferSize(
        h->handle, h->transpose, nrows, nnz, h->descr,
        reinterpret_cast<cuDoubleComplex *>(
            const_cast<Kokkos::complex<double> *>(values.data())),
    ...
    KOKKOS_CUSPARSE_SAFE_CALL(cusparseZcsrsv2_analysis(
        h->handle, h->transpose, nrows, nnz, h->descr,
        reinterpret_cast<cuDoubleComplex *>(
            const_cast<Kokkos::complex<double> *>(values.data())),
        row_map.data(), entries.data(), h->info, h->policy, h->pBuffer));
    ...
    KOKKOS_CUSPARSE_SAFE_CALL(cusparseCcsrsv2_bufferSize(
        h->handle, h->transpose, nrows, nnz, h->descr,
        reinterpret_cast<cuComplex *>(
            const_cast<Kokkos::complex<float> *>(values.data())),
        row_map.data(), entries.data(), h->info, &pBufferSize));
...
#define KOKKOSSPARSE_SPTRSV_SYMBOLIC_CUSPARSE(SCALAR, LAYOUT, MEMSPACE)        \
  ...
  struct SPTRSV_SYMBOLIC<                                                      \
    ...
    static void sptrsv_symbolic(const Kokkos::Cuda &space,                     \
      ...
      sptrsv_analysis_cusparse(space, sptrsv_handle, row_map, entries, values, \
                               trans);                                         \
      Kokkos::Profiling::popRegion();                                          \
    }                                                                          \
  };

KOKKOSSPARSE_SPTRSV_SYMBOLIC_CUSPARSE(float, Kokkos::LayoutLeft,
                                      Kokkos::CudaSpace)
KOKKOSSPARSE_SPTRSV_SYMBOLIC_CUSPARSE(double, Kokkos::LayoutLeft,
                                      Kokkos::CudaSpace)
KOKKOSSPARSE_SPTRSV_SYMBOLIC_CUSPARSE(Kokkos::complex<float>,
                                      Kokkos::LayoutLeft, Kokkos::CudaSpace)
KOKKOSSPARSE_SPTRSV_SYMBOLIC_CUSPARSE(Kokkos::complex<double>,
                                      Kokkos::LayoutLeft, Kokkos::CudaSpace)
