...
void test_extract_diagonal_blocks() {
  for (int s = 1; s <= 8; s++) {
    ...
    Test::run_test_extract_diagonal_blocks<scalar_t, lno_t, size_type, device>(3, s);
    Test::run_test_extract_diagonal_blocks<scalar_t, lno_t, size_type, device>(12, s);
    Test::run_test_extract_diagonal_blocks<scalar_t, lno_t, size_type, device>(123, s);
  }
}
