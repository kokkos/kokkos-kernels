...
void test_spmv_sell() {
  ...
  std::cout << "x: {" << x_h(0) << ", " << x_h(1) << ", " << x_h(2) << ", " << x_h(3) << ", " << x_h(4) << ", "
            << x_h(5) << ", " << x_h(6) << "}" << std::endl;
  std::cout << "y: {" << y_h(0) << ", " << y_h(1) << ", " << y_h(2) << ", " << y_h(3) << ", " << y_h(4) << ", "
            << y_h(5) << ", " << y_h(6) << "}" << std::endl;
}
}  // namespace Test

#define KOKKOSKERNELS_EXECUTE_TEST(SCALAR, ORDINAL, OFFSET, DEVICE)                         \
  TEST_F(TestCategory, sparse##_##spmv_sell##_##SCALAR##_##ORDINAL##_##OFFSET##_##DEVICE) { \
    ::Test::test_spmv_sell<SCALAR, ORDINAL, OFFSET, DEVICE>();                              \
  }
