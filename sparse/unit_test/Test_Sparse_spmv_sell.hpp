...
void test_spmv_sell_analytic() {
  ...
  Test::EXPECT_NEAR_KK_REL(y_h(0), -1, 10 * row_length * KokkosKernels::ArithTraits<scalar_t>::eps());
  Test::EXPECT_NEAR_KK_REL(y_h(1), -2, 10 * row_length * KokkosKernels::ArithTraits<scalar_t>::eps());
  Test::EXPECT_NEAR_KK_REL(y_h(2), -2, 10 * row_length * KokkosKernels::ArithTraits<scalar_t>::eps());
  Test::EXPECT_NEAR_KK_REL(y_h(3), -2, 10 * row_length * KokkosKernels::ArithTraits<scalar_t>::eps());
  Test::EXPECT_NEAR_KK_REL(y_h(4), -2, 10 * row_length * KokkosKernels::ArithTraits<scalar_t>::eps());
  Test::EXPECT_NEAR_KK_REL(y_h(5), -2, 10 * row_length * KokkosKernels::ArithTraits<scalar_t>::eps());
  Test::EXPECT_NEAR_KK_REL(y_h(6), 47, 10 * row_length * KokkosKernels::ArithTraits<scalar_t>::eps());
}
...
void test_spmv_sell(ordinal_t num_rows, ordinal_t num_cols, ordinal_t row_length, ordinal_t variance,
  ...
  for (int rowIdx = 0; rowIdx < num_rows; ++rowIdx) {
    Test::EXPECT_NEAR_KK_REL(y_h(rowIdx), y_ref_h(rowIdx), 10 * row_length * KokkosKernels::ArithTraits<scalar_t>::eps());
  }
}
