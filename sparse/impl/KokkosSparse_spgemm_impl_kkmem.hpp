...
struct KokkosSPGEMM<HandleType, a_row_view_t_, a_lno_nnz_view_t_, a_scalar_nnz_view_t_, b_lno_row_view_t_,
  ...
  PortableNumericCHASH(nnz_lno_t m_, a_row_view_t row_mapA_, a_nnz_view_t entriesA_, a_scalar_view_t valuesA_,
...
void KokkosSPGEMM<HandleType, a_row_view_t_, a_lno_nnz_view_t_, a_scalar_nnz_view_t_, b_lno_row_view_t_,
                  b_lno_nnz_view_t_, b_scalar_nnz_view_t_>::KokkosSPGEMM_numeric_hash(c_row_view_t rowmapC_,
                                                                                      c_lno_nnz_view_t entriesC_,
                                                                                      c_scalar_nnz_view_t valuesC_,
                                                                                      KokkosKernels::Impl::ExecSpaceType
                                                                                          lcl_my_exec_space) {
  ...
  if (this->spgemm_algorithm == SPGEMM_KK || SPGEMM_KK_LP == this->spgemm_algorithm) {
    if (KokkosKernels::Impl::is_gpu_exec_space_v<MyExecSpace>) {
      ...
      if (run_dense) {
        this->KokkosSPGEMM_numeric_speed(rowmapC_, entriesC_, valuesC_, lcl_my_exec_space);
        return;
      }
    }
  }
  ...
  if (KokkosKernels::Impl::is_gpu_exec_space_v<MyExecSpace>) {
    if (algorithm_to_run == SPGEMM_KK_MEMORY_SPREADTEAM) {
      ...
      Kokkos::parallel_for(
          ...
          gpu_team_policy4_t(a_row_cnt / team_row_chunk_size + 1, suggested_team_size, suggested_vector_size), sc);
      MyExecSpace().fence();
      ...
      Kokkos::parallel_for(
          ...
          gpu_team_policy6_t(a_row_cnt / team_row_chunk_size + 1, suggested_team_size, suggested_vector_size), sc);
    } else {
      if (team_shmem_key_size <= 0) {
        std::cout << "KokkosSPGEMM_numeric_hash SPGEMM_KK_MEMORY: Insufficient shmem "
                     "available for key for hash map accumulator - Terminating"
                  << std::endl;
    ...
    if (algorithm_to_run == SPGEMM_KK_LP) {
      if (use_dynamic_schedule) {
        ...
        Kokkos::parallel_for(
            "KOKKOSPARSE::SPGEMM::SPGEMM_KK_LP::STATIC",
...
void KokkosSPGEMM<HandleType, a_row_view_t_, a_lno_nnz_view_t_, a_scalar_nnz_view_t_, b_lno_row_view_t_,
                  b_lno_nnz_view_t_,
                  b_scalar_nnz_view_t_>::KokkosSPGEMM_numeric_hash2(c_row_view_t rowmapC_, c_lno_nnz_view_t entriesC_,
                                                                    c_scalar_nnz_view_t valuesC_,
  ...
  if (KOKKOSKERNELS_VERBOSE) {
    std::cout << "\t\t max_nnz: " << max_nnz << " chunk_size:" << chunksize << " numchunks:" << num_chunks << std::endl;
  }
  ...
  PortableNumericCHASH<const_a_lno_row_view_t, const_a_lno_nnz_view_t, const_a_scalar_nnz_view_t,
      ...
      sc(a_row_cnt, row_mapA, entriesA, valsA,
         ...
         rowmapC_, entriesC_, valuesC_, shmem_size, suggested_vector_size, m_space, min_hash_size, max_nnz,
         suggested_team_size,
  ...
  if (KokkosKernels::Impl::is_gpu_exec_space_v<my_exec_space>) {
    ...
    if (use_dynamic_schedule) {
      Kokkos::parallel_for("KOKKOSPARSE::SPGEMM::SPGEMM_KK_MEMORY_DYNAMIC",
                           dynamic_multicore_team_policy2_t(a_row_cnt / team_row_chunk_size + 1, suggested_team_size,
                                                            suggested_vector_size),
                           sc);
    } else {
      Kokkos::parallel_for(
          "KOKKOSPARSE::SPGEMM::SPGEMM_KK_MEMORY_STATIC",
          multicore_team_policy2_t(a_row_cnt / team_row_chunk_size + 1, suggested_team_size, suggested_vector_size),
          sc);
    }
    MyExecSpace().fence();
  }
