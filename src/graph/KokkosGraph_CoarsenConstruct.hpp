...
class coarse_builder {
 public:
  ...
  using dyn_policy_t =
      Kokkos::RangePolicy<Kokkos::Schedule<Kokkos::Dynamic>, exec_space>;
  using team_policy_t = Kokkos::TeamPolicy<exec_space>;
  using dyn_team_policy_t =
      Kokkos::TeamPolicy<Kokkos::Schedule<Kokkos::Dynamic>, exec_space>;
  ...
  static constexpr bool is_host_space = std::is_same<
  ...
  static coarse_level_triple build_coarse_graph_spgemm(
    ...
    if (handle.b == Spgemm_transpose_first) {
      ...
      vtx_view_t entries_p1("adjacencies_partial",
                            kh.get_spgemm_handle()->get_c_nnz());
      wgt_view_t values_p1("weights_partial",
                           kh.get_spgemm_handle()->get_c_nnz());
      ...
      adj_coarse =
          vtx_view_t("adjacencies_coarse", kh.get_spgemm_handle()->get_c_nnz());
      wgt_coarse =
          wgt_view_t("weights_coarse", kh.get_spgemm_handle()->get_c_nnz());
    ...
    Kokkos::parallel_for(
        policy_t(0, nc), KOKKOS_LAMBDA(ordinal_t u) {
          for (edge_offset_t j = row_map_coarse(u); j < row_map_coarse(u + 1);
               j++) {
            if (adj_coarse(j) != u) {
              nonLoops(u)++;
            }
    ...
    Kokkos::parallel_scan(
        policy_t(0, nc), KOKKOS_LAMBDA(const ordinal_t i, edge_offset_t& update,
                                       const bool final) {
    ...
    Kokkos::parallel_for(
        "compute coarse vtx wgts", policy_t(0, n),
        KOKKOS_LAMBDA(const ordinal_t i) {
  ...
  struct functorDedupeLowDegreeAfterSort {
    ...
    void operator()(const member& thread, edge_offset_t& thread_sum) const {
      ...
      Kokkos::parallel_for(
          Kokkos::TeamThreadRange(thread, start, start + dedupe_edge_count(u)),
    ...
    void operator()(const member& thread) const {
      ...
      Kokkos::parallel_for(
          ...
          [=](const edge_offset_t u_idx) {
            ...
            edge_offset_t v_dest =
                v_dest_offset +
                Kokkos::atomic_fetch_add(&target_edge_counts(v), 1);
            edge_offset_t u_dest =
                u_dest_offset +
                Kokkos::atomic_fetch_add(&target_edge_counts(u), 1);
  ...
  struct functorHashmapAccumulator {
    ...
    void operator()(const member& thread, ordinal_t& thread_sum) const {
      ...
      Kokkos::parallel_for(
          Kokkos::ThreadVectorRange(thread, row_map(idx), row_map(idx + 1)),
          [&](const edge_offset_t& i) {
            ordinal_t key  = entries_in(i);
            scalar_t value = wgts_in(i);
            ...
            int r =
                hash_map
                    .vector_atomic_insert_into_hash_mergeAtomicAdd_TrackHashes(
                        key, value, used_hash_size, used_hash_count,
                        used_hash_indices);
      ...
      Kokkos::parallel_for(
          Kokkos::ThreadVectorRange(thread, (ordinal_t)0, *used_hash_count),
          [&](const ordinal_t& i) {
            ordinal_t dirty_hash = used_hash_indices[i];
            ...
            while (end_inner != bucket) {
              ...
              bool is_sorted     = true;
              // bubble-up
              for (ordinal_t k = hash_nexts[bucket]; k != end_inner;
                   k           = hash_nexts[k]) {
                // swap
                if (keys[k] < last_key) {
            ...
            for (ordinal_t j = hash_nexts[bucket]; j != ORD_MAX;
                 j           = hash_nexts[j]) {
              if (keys[j] == key) {
                val += values[j];
              } else {
                ordinal_t write_at =
                    row_map(idx) + Kokkos::atomic_fetch_add(write_idx, 1);
      ...
      if (use_out) {
        Kokkos::parallel_for(
            Kokkos::ThreadVectorRange(thread, (ordinal_t)0, *write_idx),
            [&](const ordinal_t& i) {
  ...
  static void getHashmapSizeAndCount(
    ...
    if (!is_host_space) {
      ...
      if (mem_needed > max_mem_allowed) {
        ...
        chunk_dif        = chunk_dif /
                    (static_cast<size_t>(mem_chunk_size) * sizeof(ordinal_t));
        chunk_dif++;
  ...
  static void deduplicate_graph(coarsen_handle& handle, const ordinal_t n,
    ...
    if (handle.b == Hashmap || is_host_space) {
      ...
      Kokkos::parallel_for(
          policy_t(0, n),
          KOKKOS_LAMBDA(const ordinal_t i) { remaining(i) = i; });
      ...
      do {
        ...
        if (!is_host_space && max_entries >= 128) {
          Kokkos::parallel_reduce("hashmap time",
                                  team_policy_t(old_remaining_count, 1, 64),
                                  hashmapAccumulator, remaining_count);
        } else {
          if (use_dyn) {
            Kokkos::parallel_reduce(
                "hashmap time",
                dyn_policy_t(0, old_remaining_count, Kokkos::ChunkSize(128)),
                hashmapAccumulator, remaining_count);
          } else {
            Kokkos::parallel_reduce("hashmap time",
                                    policy_t(0, old_remaining_count),
                                    hashmapAccumulator, remaining_count);
          }
        }
      ...
      Kokkos::parallel_reduce(
          policy_t(0, n),
          KOKKOS_LAMBDA(const ordinal_t i, edge_offset_t& sum) {
      ...
      if (use_team) {
        ...
        functorDedupeAfterSort deduper(source_bucket_offset, dest_by_source,
                                       dest_out, wgt_by_source, wgts_out,
                                       edges_per_source);
        Kokkos::parallel_reduce("deduplicated sorted", team_policy_t(n, 64),
                                deduper, gc_nedges);
        ...
        Kokkos::parallel_reduce("deduplicated sorted", policy_t(0, n), deduper,
                                gc_nedges);
      }
      ...
      {
        // no thread team version
        functorDedupeLowDegreeAfterSort deduper(
            source_bucket_offset, dest_by_source, dest_by_source, wgt_by_source,
      ...
      while (remaining_count > 0) {
        ...
        if (!is_host_space && max_entries >= 128) {
          Kokkos::parallel_reduce("hashmap time",
                                  dyn_team_policy_t(old_remaining_count, 1, 64),
                                  hashmapAccumulator, remaining_count);
        } else {
          Kokkos::parallel_reduce("hashmap time",
                                  dyn_policy_t(0, old_remaining_count),
                                  hashmapAccumulator, remaining_count);
        }
  ...
  struct translationFunctor {
      ...
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(t, start, end),
                           [=](const edge_offset_t idx) {
                             ...
                             if (u != v) {
                               ...
                               edge_offset_t offset = Kokkos::atomic_fetch_add(
                                   &edges_per_source(u), 1);
    ...
    void operator()(const ordinal_t& i) const {
      ...
      for (edge_offset_t idx = start; idx < end; idx++) {
        ...
        if (u != v) {
          // fix this
          edge_offset_t offset =
              Kokkos::atomic_fetch_add(&edges_per_source(u), 1);
  ...
  static coarse_level_triple build_nonskew(coarsen_handle& handle,
    ...
    Kokkos::parallel_scan("calc source offsets", policy_t(0, nc),
                          prefix_sum(edges_per_source, source_bucket_offset));
    ...
    translationFunctor translateF(vcmap, g, mapped_edges, edges_per_source,
                                  source_bucket_offset, dest_by_source,
                                  wgt_by_source);
    if (is_host_space) {
      bool use_dyn =
      ...
      if (use_dyn) {
        ...
        Kokkos::parallel_for("move edges to coarse matrix", policy_t(0, n),
                             translateF);
      }
    } else {
      auto execSpaceEnum =
      ...
      int vectorLength = KokkosKernels::Impl::kk_get_suggested_vector_size(
          n, g.nnz(), execSpaceEnum);
      team_policy_t dummy(1, 1, vectorLength);
      int teamSize = dummy.team_size_max(translateF, Kokkos::ParallelForTag());
      Kokkos::parallel_for(
          "move edges to coarse matrix",
          team_policy_t((n + teamSize - 1) / teamSize, teamSize, vectorLength),
          translateF);
    }
    ...
    Kokkos::parallel_scan("calc source offsets again", policy_t(0, nc),
                          prefix_sum(edges_per_source, source_offsets));
    ...
    if (is_host_space) {
      ...
      if (use_dyn) {
        ...
        Kokkos::parallel_for(
            "move deduped edges to new coarse matrix", policy_t(0, nc),
            KOKKOS_LAMBDA(const ordinal_t& u) {
              edge_offset_t start_origin = source_bucket_offset(u);
              edge_offset_t start_dest   = source_offsets(u);
              for (edge_offset_t idx = 0; idx < edges_per_source(u); idx++) {
      ...
      Kokkos::parallel_for(
          "move deduped edges to new coarse matrix",
          team_policy_t(nc, Kokkos::AUTO), KOKKOS_LAMBDA(const member& thread) {
            ordinal_t u                = thread.league_rank();
            edge_offset_t start_origin = source_bucket_offset(u);
            edge_offset_t start_dest   = source_offsets(u);
            Kokkos::parallel_for(
  ...
  static matrix_t collapse_directed_to_undirected(
    ...
    Kokkos::parallel_for(
        "count directed edges owned by opposite endpoint",
        team_policy_t(nc, Kokkos::AUTO), KOKKOS_LAMBDA(const member& thread) {
          ordinal_t u         = thread.league_rank();
    ...
    Kokkos::parallel_scan("calc target row map", policy_t(0, nc),
                          prefix_sum(coarse_degree, target_row_map));

    Kokkos::deep_copy(coarse_degree, static_cast<ordinal_t>(0));

    edge_offset_t coarse_edges_total = 0;
    edge_subview_t coarse_edge_total_subview =
        Kokkos::subview(target_row_map, nc);
    ...
    Kokkos::parallel_for(
        ...
        functorCollapseDirectedToUndirected(
            source_row_map, target_row_map, source_edge_counts, coarse_degree,
  ...
  static coarse_level_triple build_skew(coarsen_handle& handle,
    ...
    Kokkos::parallel_for(
        "recount edges", team_policy_t(n, Kokkos::AUTO),
    ...
    Kokkos::parallel_for(
        "combine fine rows", team_policy_t(n, Kokkos::AUTO),
        KOKKOS_LAMBDA(const member& thread) {
          ordinal_t outer_idx = thread.league_rank();
          ordinal_t u         = vcmap.graph.entries(outer_idx);
          edge_offset_t start = g.graph.row_map(outer_idx);
    ...
    matrix_t gc = collapse_directed_to_undirected(
        nc, edges_per_source, source_bucket_offset, dest_by_source,
        wgt_by_source);
  ...
  static coarse_level_triple build_high_duplicity(coarsen_handle& handle,
    ...
    Kokkos::parallel_scan("calc source offsets", policy_t(0, n),
                          prefix_sum(dedupe_count, row_map_copy));
    // reset counters to 0
    ...
    Kokkos::parallel_for(
        "move edges to new matrix", team_policy_t(n, Kokkos::AUTO),
        KOKKOS_LAMBDA(const member& thread) {
          ordinal_t outer_idx = thread.league_rank();
          ordinal_t u         = vcmap.graph.entries(outer_idx);
          edge_offset_t start = g.graph.row_map(outer_idx);
          ...
          Kokkos::parallel_for(
              ...
              [=](const edge_offset_t idx) {
                ...
                if (u != v && (degree_less || (degree_equal && u < v))) {
                  edge_offset_t offset =
                      Kokkos::atomic_fetch_add(&dedupe_count(outer_idx), 1);
    ...
    Kokkos::parallel_for(
        "combine deduped fine rows", team_policy_t(n, Kokkos::AUTO),
        KOKKOS_LAMBDA(const member& thread) {
          ordinal_t outer_idx = thread.league_rank();
          ordinal_t u         = vcmap.graph.entries(outer_idx);
  ...
  static coarse_level_triple build_coarse_graph(coarsen_handle& handle,
    ...
    countingFunctor countF(vcmap, g, mapped_edges, degree_initial, c_vtx_w,
                           f_vtx_w);
    if (is_host_space) {
      Kokkos::parallel_for(
          "count edges per coarse vertex (also compute coarse vertex weights)",
          policy_t(0, n), countF);
    } else {
      auto execSpaceEnum =
          KokkosKernels::Impl::kk_get_exec_space_type<exec_space>();
