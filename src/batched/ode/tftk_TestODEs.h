...
struct CosExp
{
  ...
  KOKKOS_FUNCTION void derivatives(double t, View1 & y, View2 & dydt) const
  {
    for (int i = 0; i < neqs; i++)
    {
      dydt[i] = lambda * (y[i] - Kokkos::Experimental::cos(t)) - Kokkos::Experimental::sin(t);
    }
  }
  ...
  KOKKOS_FUNCTION double expected_val(const double t, const int n) const
  {
    return Kokkos::Experimental::exp(lambda * (t - t0)) * (eta - Kokkos::Experimental::cos(t0)) +
...
// Solution is of the form y_j = cj1 * exp(-K1 * t) + cj2 * exp(-K2 * t) + cj3 for j = 0, 1, 2
struct StiffChemicalDecayProcess
{
  StiffChemicalDecayProcess(int neqs_, double K1_, double K2_) : neqs(neqs_), K1(K1_), K2(K2_) {}
...
struct Tracer
{
  ...
  KOKKOS_FUNCTION void derivatives(double t, View1 & y, View2 & dydt) const
  {
    ...
    {
      const double R = Kokkos::Experimental::sqrt(y[i] * y[i] + y[i + 1] * y[i + 1]);
      dydt[i] = -rate * y[i + 1] / R;
      dydt[i + 1] = rate * y[i] / R;
    }
  }
