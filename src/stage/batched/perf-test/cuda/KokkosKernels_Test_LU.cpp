...
#define FLOP_ADD 1.0
            ...
            Kokkos::parallel_for
              (Kokkos::ThreadVectorRange(member, VectorLength),
               [&](const int &k) {
                ...
                if (kk < _a.dimension_0()) {
                  auto aa = Kokkos::subview(_a, kk, Kokkos::ALL(), Kokkos::ALL());
                  Team::LU<MemberType,AlgoTagType>::invoke(member, aa);
                }
              });
          }
        ...
        void LU(const int N, const int BlkSize) {
...
void run(const int N, const int B) {
  ...
  if (B != 0) {
    PerfTest::LU<VectorLength, ValueType, ExecSpace, AlgoTagType>(N,B);
  } else {
    PerfTest::LU<VectorLength, ValueType, ExecSpace, AlgoTagType>(N, 3);
    PerfTest::LU<VectorLength, ValueType, ExecSpace, AlgoTagType>(N, 5);
    PerfTest::LU<VectorLength, ValueType, ExecSpace, AlgoTagType>(N,10);
    PerfTest::LU<VectorLength, ValueType, ExecSpace, AlgoTagType>(N,15);
  }
}
...
int main(int argc, char *argv[]) {
...

  {
    ...
    std::cout << "\n Testing LayoutLeft-" << VectorLength << " and Algo::LU::Unblocked\n";
    run<VectorLength,double,Algo::LU::Unblocked>(N/VectorLength,B);
    
    std::cout << "\n Testing LayoutLeft-" << VectorLength << " and Algo::LU::Blocked\n";
    run<VectorLength,double,Algo::LU::Blocked>(N/VectorLength,B);
  }
