...
#define FLOP_ADD 1.0
        ...
        //       // TODO:: league size should be properly computed via chunk per team
        //       // if we assume N = leaguesize then this is same as dynamic scheduling.
...
void run(const int N) {
  ...
  PerfTest::Gemm< 3, ExecSpace,VectorType,AlgoTagType>(N);
  PerfTest::Gemm< 5, ExecSpace,VectorType,AlgoTagType>(N);
  PerfTest::Gemm<10, ExecSpace,VectorType,AlgoTagType>(N);
  PerfTest::Gemm<15, ExecSpace,VectorType,AlgoTagType>(N);
}
