...
void potrs(const execution_space& space,
                const char uplo[],
                const int& n,
                const int& nrhs,
                const AViewType& A,
  ...
  static_assert(Kokkos::is_view<AViewType>::value,
                "KokkosLapack::potrs: AViewType must be a Kokkos::View");
  static_assert(Kokkos::is_view<BViewType>::value,
                "KokkosLapack::potrs: BViewType must be a Kokkos::View");
  ...
  Impl::Potrs<AViewInternalType, BViewInternalType>::potrs(uplo, n, nrhs, uA, lda, uB, ldb);
}
