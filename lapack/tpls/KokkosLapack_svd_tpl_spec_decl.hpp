...
inline void svd_print_specialization() {
  ...
  if constexpr (std::is_same_v<ExecutionSpace, Kokkos::Cuda>) {
    printf(
        ...
        ">\n",
        typeid(AMatrix).name(), typeid(SVector).name(), typeid(UMatrix).name(),
        typeid(VMatrix).name());
  }
#endif
#endif
...
template <class ExecutionSpace, class AMatrix, class SVector, class UMatrix,
          class VMatrix>
void lapackSvdWrapper(const ExecutionSpace& /* space */, const char jobu[],
  ...
  if constexpr (Kokkos::ArithTraits<Scalar>::is_complex) {
    HostLapack<std::complex<Magnitude>>::gesvd(
        jobu[0], jobvt[0], m, n,
        reinterpret_cast<std::complex<Magnitude>*>(A.data()), lda, S.data(),
        reinterpret_cast<std::complex<Magnitude>*>(U.data()), ldu,
        reinterpret_cast<std::complex<Magnitude>*>(Vt.data()), ldvt,
        reinterpret_cast<std::complex<Magnitude>*>(work.data()), lwork,
        rwork.data(), info);
...
KOKKOSLAPACK_SVD_LAPACK(Kokkos::complex<float>, Kokkos::LayoutLeft,
                        Kokkos::Serial)
KOKKOSLAPACK_SVD_LAPACK(Kokkos::complex<double>, Kokkos::LayoutLeft,
                        Kokkos::Serial)
#endif
...
void mklSvdWrapper(const ExecutionSpace& /* space */, const char jobu[],
  ...
  if constexpr (std::is_same_v<Scalar, float>) {
    ret =
        LAPACKE_sgesvd(LAPACK_COL_MAJOR, jobu[0], jobvt[0], m, n, A.data(), lda,
                       S.data(), U.data(), ldu, Vt.data(), ldvt, rwork.data());
  }
  if constexpr (std::is_same_v<Scalar, double>) {
    ret =
        LAPACKE_dgesvd(LAPACK_COL_MAJOR, jobu[0], jobvt[0], m, n, A.data(), lda,
                       S.data(), U.data(), ldu, Vt.data(), ldvt, rwork.data());
  }
  if constexpr (std::is_same_v<Scalar, Kokkos::complex<float>>) {
    ret = LAPACKE_cgesvd(
        LAPACK_COL_MAJOR, jobu[0], jobvt[0], m, n,
        reinterpret_cast<lapack_complex_float*>(A.data()), lda, S.data(),
        reinterpret_cast<lapack_complex_float*>(U.data()), ldu,
        reinterpret_cast<lapack_complex_float*>(Vt.data()), ldvt, rwork.data());
  }
  if constexpr (std::is_same_v<Scalar, Kokkos::complex<double>>) {
    ret = LAPACKE_zgesvd(
        LAPACK_COL_MAJOR, jobu[0], jobvt[0], m, n,
        reinterpret_cast<lapack_complex_double*>(A.data()), lda, S.data(),
...
template <class ExecutionSpace, class AMatrix, class SVector, class UMatrix,
          class VMatrix>
void cusolverSvdWrapper(const ExecutionSpace& space, const char jobu[],
                        const char jobvt[], const AMatrix& A, const SVector& S,
                        const UMatrix& U, const VMatrix& Vt) {
  using memory_space = typename AMatrix::memory_space;
  using Scalar       = typename AMatrix::non_const_value_type;
  ...
  if constexpr (std::is_same_v<Scalar, float>) {
    KOKKOS_CUSOLVER_SAFE_CALL_IMPL(
        cusolverDnSgesvd_bufferSize(s.handle, m, n, &lwork));
    Kokkos::View<Scalar*, memory_space> work("svd work buffer", lwork);

    KOKKOS_CUSOLVER_SAFE_CALL_IMPL(cusolverDnSgesvd(
        s.handle, jobu[0], jobvt[0], m, n, A.data(), lda, S.data(), U.data(),
        ldu, Vt.data(), ldvt, work.data(), lwork, rwork.data(), info.data()));
  }
  if constexpr (std::is_same_v<Scalar, double>) {
    KOKKOS_CUSOLVER_SAFE_CALL_IMPL(
        cusolverDnDgesvd_bufferSize(s.handle, m, n, &lwork));
    Kokkos::View<Scalar*, memory_space> work("svd work buffer", lwork);

    KOKKOS_CUSOLVER_SAFE_CALL_IMPL(cusolverDnDgesvd(
        s.handle, jobu[0], jobvt[0], m, n, A.data(), lda, S.data(), U.data(),
        ldu, Vt.data(), ldvt, work.data(), lwork, rwork.data(), info.data()));
  }
  if constexpr (std::is_same_v<Scalar, Kokkos::complex<float>>) {
    KOKKOS_CUSOLVER_SAFE_CALL_IMPL(
        cusolverDnCgesvd_bufferSize(s.handle, m, n, &lwork));
    Kokkos::View<Scalar*, memory_space> work("svd work buffer", lwork);

    KOKKOS_CUSOLVER_SAFE_CALL_IMPL(
        cusolverDnCgesvd(s.handle, jobu[0], jobvt[0], m, n,
                         reinterpret_cast<cuComplex*>(A.data()), lda, S.data(),
                         reinterpret_cast<cuComplex*>(U.data()), ldu,
                         reinterpret_cast<cuComplex*>(Vt.data()), ldvt,
                         reinterpret_cast<cuComplex*>(work.data()), lwork,
                         rwork.data(), info.data()));
  }
  if constexpr (std::is_same_v<Scalar, Kokkos::complex<double>>) {
    KOKKOS_CUSOLVER_SAFE_CALL_IMPL(
        cusolverDnZgesvd_bufferSize(s.handle, m, n, &lwork));
    Kokkos::View<Scalar*, memory_space> work("svd work buffer", lwork);

    KOKKOS_CUSOLVER_SAFE_CALL_IMPL(
        cusolverDnZgesvd(s.handle, jobu[0], jobvt[0], m, n,
                         reinterpret_cast<cuDoubleComplex*>(A.data()), lda,
                         S.data(), reinterpret_cast<cuDoubleComplex*>(U.data()),
                         ldu, reinterpret_cast<cuDoubleComplex*>(Vt.data()),
                         ldvt, reinterpret_cast<cuDoubleComplex*>(work.data()),
                         lwork, rwork.data(), info.data()));
  }
  ...
  struct SVD<                                                                 \
      ...
      svd_eti_spec_avail<                                                     \
          ...
          Kokkos::View<SCALAR**, LAYOUT,                                      \
                       Kokkos::Device<Kokkos::Cuda, MEM_SPACE>,               \
                       Kokkos::MemoryTraits<Kokkos::Unmanaged>>>::value> {    \
    using AMatrix = Kokkos::View<SCALAR**, LAYOUT,                            \
                                 Kokkos::Device<Kokkos::Cuda, MEM_SPACE>,     \
                                 Kokkos::MemoryTraits<Kokkos::Unmanaged>>;    \
    using SVector =                                                           \
        Kokkos::View<Kokkos::ArithTraits<SCALAR>::mag_type*, LAYOUT,          \
                     Kokkos::Device<Kokkos::Cuda, MEM_SPACE>,                 \
                     Kokkos::MemoryTraits<Kokkos::Unmanaged>>;                \
    using UMatrix = Kokkos::View<SCALAR**, LAYOUT,                            \
                                 Kokkos::Device<Kokkos::Cuda, MEM_SPACE>,     \
                                 Kokkos::MemoryTraits<Kokkos::Unmanaged>>;    \
    using VMatrix = Kokkos::View<SCALAR**, LAYOUT,                            \
                                 Kokkos::Device<Kokkos::Cuda, MEM_SPACE>,     \
                                 Kokkos::MemoryTraits<Kokkos::Unmanaged>>;    \
                                                                              \
    static void svd(const Kokkos::Cuda& space, const char jobu[],             \
                    const char jobvt[], const AMatrix& A, const SVector& S,   \
                    const UMatrix& U, const VMatrix& Vt) {                    \
      Kokkos::Profiling::pushRegion("KokkosLapack::svd[TPL_CUSOLVER," #SCALAR \
                                    "]");                                     \
      svd_print_specialization<Kokkos::Cuda, AMatrix, SVector, UMatrix,       \
                               VMatrix>();                                    \
                                                                              \
...
KOKKOSLAPACK_SVD_CUSOLVER(Kokkos::complex<float>, Kokkos::LayoutLeft,
                          Kokkos::CudaSpace)
KOKKOSLAPACK_SVD_CUSOLVER(Kokkos::complex<double>, Kokkos::LayoutLeft,
                          Kokkos::CudaSpace)
...
template <class ExecutionSpace, class AMatrix, class SVector, class UMatrix,
          class VMatrix>
void rocsolverSvdWrapper(const ExecutionSpace& space, const char jobu[],
                         const char jobvt[], const AMatrix& A, const SVector& S,
                         const UMatrix& U, const VMatrix& Vt) {
  using memory_space = typename AMatrix::memory_space;
  using Scalar       = typename AMatrix::non_const_value_type;
  ...
  if constexpr (std::is_same_v<Scalar, float>) {
    KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocsolver_sgesvd(
        s.handle, UVecMode, VVecMode, m, n, A.data(), lda, S.data(), U.data(),
        ldu, Vt.data(), ldvt, rwork.data(), WorkMode, info.data()));
  }
  if constexpr (std::is_same_v<Scalar, double>) {
    KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocsolver_dgesvd(
        s.handle, UVecMode, VVecMode, m, n, A.data(), lda, S.data(), U.data(),
        ldu, Vt.data(), ldvt, rwork.data(), WorkMode, info.data()));
  }
  if constexpr (std::is_same_v<Scalar, Kokkos::complex<float>>) {
    KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocsolver_cgesvd(
        s.handle, UVecMode, VVecMode, m, n,
        reinterpret_cast<rocblas_float_complex*>(A.data()), lda, S.data(),
        reinterpret_cast<rocblas_float_complex*>(U.data()), ldu,
        reinterpret_cast<rocblas_float_complex*>(Vt.data()), ldvt, rwork.data(),
        WorkMode, info.data()));
  }
  if constexpr (std::is_same_v<Scalar, Kokkos::complex<double>>) {
    KOKKOS_ROCBLAS_SAFE_CALL_IMPL(rocsolver_zgesvd(
        s.handle, UVecMode, VVecMode, m, n,
        reinterpret_cast<rocblas_double_complex*>(A.data()), lda, S.data(),
        reinterpret_cast<rocblas_double_complex*>(U.data()), ldu,
        reinterpret_cast<rocblas_double_complex*>(Vt.data()), ldvt,
        rwork.data(), WorkMode, info.data()));
  }
  ...
  struct SVD<                                                                  \
    ...
    using VMatrix =                                                            \
        Kokkos::View<SCALAR**, LAYOUT, Kokkos::Device<Kokkos::HIP, MEM_SPACE>, \
                     Kokkos::MemoryTraits<Kokkos::Unmanaged>>;                 \
                                                                               \
    static void svd(const Kokkos::HIP& space, const char jobu[],               \
                    const char jobvt[], const AMatrix& A, const SVector& S,    \
                    const UMatrix& U, const VMatrix& Vt) {                     \
      Kokkos::Profiling::pushRegion("KokkosLapack::svd[TPL_ROCSOLVER," #SCALAR \
                                    "]");                                      \
      svd_print_specialization<Kokkos::HIP, AMatrix, SVector, UMatrix,         \
                               VMatrix>();                                     \
                                                                               \
...
KOKKOSLAPACK_SVD_ROCSOLVER(Kokkos::complex<float>, Kokkos::LayoutLeft,
                           Kokkos::HIPSpace)
KOKKOSLAPACK_SVD_ROCSOLVER(Kokkos::complex<double>, Kokkos::LayoutLeft,
                           Kokkos::HIPSpace)
