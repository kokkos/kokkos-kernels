...
#define KOKKOSLAPACK_TRTRI_LAPACK_HOST(SCALAR_TYPE, BASE_SCALAR_TYPE, LAYOUTA, \
  ...
  struct TRTRI<Kokkos::View<int, Kokkos::LayoutRight, Kokkos::HostSpace,       \
    ...
    static void trtri(const RViewType& R, const char uplo[],                   \
      ...
      Kokkos::Profiling::pushRegion(                                           \
          "KokkosLapack::trtri[TPL_LAPACK," #SCALAR_TYPE "]");                 \
      ...
      if ((uplo[0] == 'L') || (uplo[0] == 'l'))                                \
        ...
        uplo_ = A_is_layout_left ? 'U' : 'L';                                  \
                                                                               \
      R() = HostLapack<BASE_SCALAR_TYPE>::trtri(                               \
          uplo_, diag[0], M,                                                   \
...
#define KOKKOSLAPACK_TRTRI_LAPACK_MAGMA(SCALAR_TYPE, BASE_SCALAR_TYPE,   \
                                        MAGMA_FN, LAYOUTA, MEM_SPACE,    \
                                        ETI_SPEC_AVAIL)                  \
  template <class ExecSpace>                                             \
  struct TRTRI<Kokkos::View<int, Kokkos::LayoutRight, Kokkos::HostSpace, \
                            Kokkos::MemoryTraits<Kokkos::Unmanaged> >,   \
...
#define KOKKOSLAPACK_STRTRI_LAPACK(LAYOUTA, ETI_SPEC_AVAIL)                   \
  KOKKOSLAPACK_TRTRI_LAPACK_HOST(float, float, LAYOUTA, Kokkos::HostSpace,    \
                                 ETI_SPEC_AVAIL)                              \
  KOKKOSLAPACK_TRTRI_LAPACK_MAGMA(float, magmaFloat_ptr, magma_strtri_gpu,    \
                                  LAYOUTA, Kokkos::CudaSpace, ETI_SPEC_AVAIL) \
  KOKKOSLAPACK_TRTRI_LAPACK_MAGMA(float, magmaFloat_ptr, magma_strtri_gpu,    \
                                  LAYOUTA, Kokkos::CudaUVMSpace,              \
                                  ETI_SPEC_AVAIL)
...
#define KOKKOSLAPACK_ZTRTRI_LAPACK(LAYOUTA, ETI_SPEC_AVAIL)                   \
  ...
  KOKKOSLAPACK_TRTRI_LAPACK_MAGMA(Kokkos::complex<double>,                    \
                                  magmaDoubleComplex_ptr, magma_ztrtri_gpu,   \
                                  LAYOUTA, Kokkos::CudaSpace, ETI_SPEC_AVAIL) \
  KOKKOSLAPACK_TRTRI_LAPACK_MAGMA(                                            \
      Kokkos::complex<double>, magmaDoubleComplex_ptr, magma_ztrtri_gpu,      \
...
#define KOKKOSLAPACK_CTRTRI_LAPACK(LAYOUTA, ETI_SPEC_AVAIL)                   \
  ...
  KOKKOSLAPACK_TRTRI_LAPACK_MAGMA(Kokkos::complex<float>,                     \
                                  magmaFloatComplex_ptr, magma_ctrtri_gpu,    \
                                  LAYOUTA, Kokkos::CudaSpace, ETI_SPEC_AVAIL) \
  KOKKOSLAPACK_TRTRI_LAPACK_MAGMA(                                            \
