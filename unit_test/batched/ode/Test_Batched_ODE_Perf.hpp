...
void scratch_kernel(const ODEType & ode, const SolverType & solver, const int nelems)
{
  ...
  const int team_size = std::is_same<MemorySpace, Kokkos::HostSpace>::value ? 1 : 32;
  const int scratch_size = ScratchView::shmem_size(team_size, (4 + SolverType::nstages), ode.neqs);

  using member_type =
      typename Kokkos::TeamPolicy<typename MemorySpace::execution_space>::member_type;
  auto team_policy =
      Kokkos::TeamPolicy<typename MemorySpace::execution_space>(nelems / team_size, team_size);
  team_policy.set_scratch_size(0, Kokkos::PerTeam(scratch_size), Kokkos::PerThread(0));
...
void kernel(const RkDynamicAllocation<MemorySpace> & pool,
    const ODEType & ode,
...
template <typename MemorySpace, typename ODEType, typename SolverType>
void scratch_perf_run(const ODEType & ode, const SolverType & solver, const int nelems)
{
  scratch_kernel<MemorySpace>(ode, solver, nelems);
}

template <typename MemorySpace, typename SolverState, typename ODEType, typename SolverType>
void perf_run(RkDynamicAllocation<MemorySpace> & pool,
    const ODEType & ode,
    const SolverType & solver,
    const int nelems)
{
...
struct RKPerfTest
{
  ...
  RKPerfTest(
      const int nelems, const ODEArgs & args, const ODEType & ode_, const bool use_stack)
...
{
  ODEArgs args;
  RKPerfTest<MemorySpace, TableType, EnrightB5, 6> t1(nelems, args, EnrightB5(6), use_stack);
  RKPerfTest<MemorySpace, TableType, EnrightC1, 4> t2(nelems, args, EnrightC1(4), use_stack);
  RKPerfTest<MemorySpace, TableType, EnrightC5, 4> t3(nelems, args, EnrightC5(4), use_stack);
}

template <typename MemorySpace, typename TableType> void run_enright_scratch(const int nelems)
{
  ODEArgs args;
  RKScratchPerfTest<MemorySpace, TableType, EnrightB5> t1(nelems, args, EnrightB5(6));
  RKScratchPerfTest<MemorySpace, TableType, EnrightC1> t2(nelems, args, EnrightC1(4));
  RKScratchPerfTest<MemorySpace, TableType, EnrightC5> t3(nelems, args, EnrightC5(4));
}

template <typename MemorySpace> void run_all_tables_scratch(const int nelems)
{
...
template <typename MemorySpace> void run_all_tables(const bool use_stack, const int nelems)
{
...
TEST_F(TestCategory, ODE_RKPerformance)
{
  ...
  if (1
    ...
    // But the original test with Kokkos::Host was giving valid info.... Hmm. What was Host at the time? 
  {
    {
      Kokkos::Timer timer;
      run_all_tables_scratch<TestExecSpace>(nelems_device);
      dt_device_scratch = timer.seconds();
      std::cout << "RK Performance - Device (Scratch) time = " << dt_device_scratch << "\n";
    }

    {
      ...
      std::cout << "RK Performance - Device (Dynamic) time = " << dt_device_dynamic << "\n";
    }
    {
      Kokkos::Timer timer;
    ...
    std::cout << "RK Performance - Host / Device (Stack) time = " << dt_host_stack / dt_device_stack
              << "\n";
  }

  std::cout << "RK Performance - Host (Dynamic / Stack) time = " << dt_host_dynamic / dt_host_stack
            << "\n";
  std::cout << "RK Performance - Host (Scratch / Stack) time = " << dt_host_scratch / dt_host_stack
            << "\n";
}
