...
//#include "KokkosBatched_BatchedGemm_Impl.hpp"
...
namespace Test {
  ...
  void impl_test_batched_gemm(const int N, const int matAdim1, const int matAdim2, const int matBdim1, const int matBdim2, const int matCdim1, const int matCdim2) {
    ...
    try {
      ret = BatchedGemm<transA, transB, batchLayout>::invoke(alpha, a_actual, b_actual, beta, c_actual); // Compute c_actual
    } catch (const std::runtime_error& error) {
      if(!((std::is_same<view_layout, Kokkos::LayoutLeft>::value  && !std::is_same<batchLayout, BatchLayout::Right>::value) ||
        (std::is_same<view_layout, Kokkos::LayoutRight>::value && !std::is_same<batchLayout, BatchLayout::Left>::value))) {
        FAIL();
      }
      return;
    }
...
int test_batched_gemm() {
#if defined(KOKKOSKERNELS_INST_LAYOUTLEFT)
  {
    ...
    Test::impl_test_batched_gemm<DeviceType,ViewType,ScalarType,ParamTagType>(0, 10, 10, 10, 10, 10, 10);
    for (int i=0;i<10;++i) {
      //printf("Testing: LayoutLeft,  Blksize %d\n", i);
      Test::impl_test_batched_gemm<DeviceType,ViewType,ScalarType,ParamTagType>(1024, i, i, i, i, i, i);
    }
    for (int i=0;i<10;++i) {
      //printf("Testing: LayoutLeft,  Blksize %d\n", i);
      ...
      if ((std::is_same<typename ParamTagType::transA,KokkosBatched::Trans::NoTranspose>::value) &&
        (std::is_same<typename ParamTagType::transB,KokkosBatched::Trans::NoTranspose>::value)) {
          Test::impl_test_batched_gemm<DeviceType,ViewType,ScalarType,ParamTagType>(1024, dimM, dimK, dimK, dimN, dimM, dimN); }
      if ((std::is_same<typename ParamTagType::transA,KokkosBatched::Trans::NoTranspose>::value) &&
        (std::is_same<typename ParamTagType::transB,KokkosBatched::Trans::Transpose>::value)) {
          Test::impl_test_batched_gemm<DeviceType,ViewType,ScalarType,ParamTagType>(1024, dimM, dimK, dimN, dimK, dimM, dimN); }
      if ((std::is_same<typename ParamTagType::transA,KokkosBatched::Trans::Transpose>::value) &&
        (std::is_same<typename ParamTagType::transB,KokkosBatched::Trans::NoTranspose>::value)) {
          Test::impl_test_batched_gemm<DeviceType,ViewType,ScalarType,ParamTagType>(1024, dimK, dimM, dimK, dimN, dimM, dimN); }
      if ((std::is_same<typename ParamTagType::transA,KokkosBatched::Trans::Transpose>::value) &&
        (std::is_same<typename ParamTagType::transB,KokkosBatched::Trans::Transpose>::value)) {
          Test::impl_test_batched_gemm<DeviceType,ViewType,ScalarType,ParamTagType>(1024, dimK, dimM, dimN, dimK, dimM, dimN); }
    }
  }
#endif
