...
#include <KokkosBlas2_teamvector_gemv.hpp>
#include <KokkosBlas1_dot.hpp>
#include <KokkosKernels_TestUtils.hpp>

namespace Test {
template <class ViewTypeA, class ViewTypeX, class ViewTypeY, class Device>
void impl_test_teamvector_gemv(const char *mode, int N, int M) {
  ...
  const int team_data_size = (N % K == 0) ? (N / K) : (N / K + 1);
  ...
  const auto team_rows = [&](auto teamId) {
    return Kokkos::make_pair(
        teamId * team_data_size,
        (teamId < K - 1) ? (teamId + 1) * team_data_size : N);
  };

  // KokkosBlas::gemv(mode,a,A,x,b,y);
  Kokkos::parallel_for(
      "KokkosBlas::Test::TeamVectorGemm", policy,
      KOKKOS_LAMBDA(const team_member &teamMember) {
        const int teamId = teamMember.league_rank();
        KokkosBlas::Experimental::team_vector_gemv(
            teamMember, trans, a,
            Kokkos::subview(A, team_rows(teamId), Kokkos::ALL()), x, b,
            Kokkos::subview(y, team_rows(teamId)));
      });
  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const team_member &teamMember) {
        ...
        KokkosBlas::Experimental::team_vector_gemv(
            teamMember, trans, a,
            Kokkos::subview(A, team_rows(teamId), Kokkos::ALL()), c_x, b,
            Kokkos::subview(y, team_rows(teamId)));
      });
  ...
  Kokkos::parallel_for(
      ...
      KOKKOS_LAMBDA(const team_member &teamMember) {
        ...
        KokkosBlas::Experimental::team_vector_gemv(
            teamMember, trans, a,
            Kokkos::subview(c_A, team_rows(teamId), Kokkos::ALL()), c_x, b,
            Kokkos::subview(y, team_rows(teamId)));
      });
...
int test_teamvector_gemv(const char *mode) {
  ...
  Test::impl_test_teamvector_gemv<view_type_a_ll, view_type_b_ll,
                                  view_type_c_ll, Device>(mode, 0, 1024);
  Test::impl_test_teamvector_gemv<view_type_a_ll, view_type_b_ll,
                                  view_type_c_ll, Device>(mode, 13, 1024);
  Test::impl_test_teamvector_gemv<view_type_a_ll, view_type_b_ll,
                                  view_type_c_ll, Device>(mode, 124, 124);
  // Test::impl_test_teamvector_gemv<view_type_a_ll, view_type_b_ll,
  // view_type_c_ll, Device>(mode,132231,1024);
#endif
  ...
  Test::impl_test_teamvector_gemv<view_type_a_lr, view_type_b_lr,
                                  view_type_c_lr, Device>(mode, 0, 1024);
  Test::impl_test_teamvector_gemv<view_type_a_lr, view_type_b_lr,
                                  view_type_c_lr, Device>(mode, 13, 1024);
  Test::impl_test_teamvector_gemv<view_type_a_lr, view_type_b_lr,
                                  view_type_c_lr, Device>(mode, 124, 124);
  // Test::impl_test_teamvector_gemv<view_type_a_lr, view_type_b_lr,
  // view_type_c_lr, Device>(mode,132231,1024);
#endif
  ...
  Test::impl_test_teamvector_gemv<view_type_a_ls, view_type_b_ls,
                                  view_type_c_ls, Device>(mode, 0, 1024);
  Test::impl_test_teamvector_gemv<view_type_a_ls, view_type_b_ls,
                                  view_type_c_ls, Device>(mode, 13, 1024);
  Test::impl_test_teamvector_gemv<view_type_a_ls, view_type_b_ls,
                                  view_type_c_ls, Device>(mode, 124, 124);
  // Test::impl_test_teamvector_gemv<view_type_a_ls, view_type_b_ls,
  // view_type_c_ls, Device>(mode,132231,1024);
    ...
    !defined(KOKKOSKERNELS_IMPL_CHECK_ETI_CALLS)
  Test::impl_test_teamvector_gemv<view_type_a_ls, view_type_b_ll,
                                  view_type_c_lr, Device>(mode, 124, 124);
  Test::impl_test_teamvector_gemv<view_type_a_ll, view_type_b_ls,
                                  view_type_c_lr, Device>(mode, 124, 124);
#endif
  ...
  return 1;
    ...
    (!defined(KOKKOSKERNELS_ETI_ONLY) && \
     !defined(KOKKOSKERNELS_IMPL_CHECK_ETI_CALLS))
TEST_F(TestCategory, teamvector_gemv_float) {
  test_teamvector_gemv<float, float, float, TestExecSpace>("N");
}
#endif
    ...
    (!defined(KOKKOSKERNELS_ETI_ONLY) &&  \
     !defined(KOKKOSKERNELS_IMPL_CHECK_ETI_CALLS))
TEST_F(TestCategory, teamvector_gemv_double) {
  test_teamvector_gemv<double, double, double, TestExecSpace>("N");
}
#endif
    ...
    (!defined(KOKKOSKERNELS_ETI_ONLY) &&          \
     !defined(KOKKOSKERNELS_IMPL_CHECK_ETI_CALLS))
TEST_F(TestCategory, teamvector_gemv_complex_double) {
  test_teamvector_gemv<Kokkos::complex<double>, Kokkos::complex<double>,
    ...
    (!defined(KOKKOSKERNELS_ETI_ONLY) && \
     !defined(KOKKOSKERNELS_IMPL_CHECK_ETI_CALLS))
TEST_F(TestCategory, teamvector_gemv_int) {
  test_teamvector_gemv<int, int, int, TestExecSpace>("N");
}
#endif
...
#if !defined(KOKKOSKERNELS_ETI_ONLY) && \
    !defined(KOKKOSKERNELS_IMPL_CHECK_ETI_CALLS)
TEST_F(TestCategory, teamvector_gemv_double_int) {
  test_teamvector_gemv<double, int, float, TestExecSpace>("N");
}
#endif
#endif
