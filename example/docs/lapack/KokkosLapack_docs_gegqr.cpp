...
int main(void) {
  ...
  Kokkos::initialize();
  {
    ...
    h_A(0, 0) = 12; h_A(0, 1) = -51; h_A(0, 2) =   4;
    h_A(1, 0) =  6; h_A(1, 1) = 167; h_A(1, 2) = -68;
    h_A(2, 0) = -4; h_A(2, 1) =  24; h_A(2, 2) = -41;
    ...
    analytic(0, 0) = -6./7; analytic(0, 1) = 69./175; analytic(0, 2) = 58./175;
    analytic(1, 0) = -3./7; analytic(1, 1) = -158./175; analytic(1, 2) = -6./175;
    analytic(2, 0) = 2./7; analytic(2, 1) = -6./35; analytic(2, 2) = 33./35;

    for (int rowIdx = 0; rowIdx < h_A.extent_int(0); ++rowIdx) {
      for (int colIdx = 0; colIdx < h_A.extent_int(1); ++colIdx) {
	if (KAT::abs(h_A(rowIdx, colIdx) - analytic(rowIdx, colIdx)) > 10*KAT::abs(analytic(rowIdx, colIdx))*KAT::epsilon()) {
	  std::cout << "h_A("<< rowIdx << ", " << colIdx << ")=" << h_A(rowIdx, colIdx)
		    << ", analytic is " << analytic(rowIdx, colIdx) << std::endl;
	  correct = false;
	}
      }
    }
