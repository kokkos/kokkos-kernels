...
int main(void) {
  ...
  Kokkos::initialize();
  {
    ...
    analytic(1, 0) = 0; analytic(1, 1) = -175; analytic(1, 2) = 70;
    analytic(2, 0) = 0; analytic(2, 1) = 0; analytic(2, 2) = -35;
	...
	if (colIdx < rowIdx) {
	  if (KAT::abs(h_R(rowIdx, colIdx)) > 100*KAT::epsilon()) {
	    std::cout << "h_R("<< rowIdx << ", " << colIdx << ")=" << h_R(rowIdx, colIdx)
		      << ", analytic is " << analytic(rowIdx, colIdx) << std::endl;
	    correct = false;
	  }
	} else if (KAT::abs(h_R(rowIdx, colIdx) - analytic(rowIdx, colIdx)) > KAT::abs(analytic(rowIdx, colIdx))*KAT::epsilon()) {
	  std::cout << "h_R("<< rowIdx << ", " << colIdx << ")=" << h_R(rowIdx, colIdx)
		    << ", analytic is " << analytic(rowIdx, colIdx) << std::endl;
	  correct = false;
	}
