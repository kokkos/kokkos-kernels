...
int main(void) {
  ...
  Kokkos::initialize();
  {
    ...
    analytic(0, 0) = -14; analytic(0, 1) = -21; analytic(0, 2) = 14;
    analytic(1, 0) = 6./26; analytic(1, 1) = -175; analytic(1, 2) = 70;
    analytic(2, 0) = -4./26; analytic(2, 1) = 1./18; analytic(2, 2) = -35;

    for (int rowIdx = 0; rowIdx < A.extent_int(0); ++rowIdx) {
      for (int colIdx = 0; colIdx < A.extent_int(1); ++colIdx) {
	if (KAT::abs(h_A(rowIdx, colIdx) - analytic(rowIdx, colIdx)) > KAT::abs(analytic(rowIdx, colIdx))*KAT::epsilon()) {
