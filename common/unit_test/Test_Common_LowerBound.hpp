...
size_t std_lower_bound(const std::vector<Ordinal> &haystack,
...
struct ThreadLowerBoundFunctor {
  ...
  ThreadLowerBoundFunctor(const hv_size_type &expected,
...
void test_lower_bound_thread(const std::vector<T> &_haystack,
                             const T &_needle) {
  ...
  Kokkos::parallel_reduce(Policy(0, 1),
                          ThreadLowerBoundFunctor(expected, haystack, _needle),
                          errCount);
...
struct TeamLowerBoundFunctor {
  ...
  TeamLowerBoundFunctor(const hv_size_type &expected,
                        const HaystackView &haystack,
                        const hv_value_type &needle)
      : expected_(expected), haystack_(haystack), needle_(needle) {}
...
void test_lower_bound_team(const std::vector<T> &_haystack, const T _needle) {
  ...
  Kokkos::parallel_reduce(
      ...
      TeamLowerBoundFunctor<Member, view_t>(expected, haystack, _needle),
      errCount);
...
#define EXECUTE_TEST(T, DEVICE)                                   \
  TEST_F(TestCategory, common##_##lower_bound##_##T##_##DEVICE) { \
    test_lower_bound<T, DEVICE>();                                \
  }
