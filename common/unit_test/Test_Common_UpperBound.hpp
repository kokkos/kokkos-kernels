...
/// \file Test_Common_UpperBound.hpp
/// \brief Tests upper bounds search routines
...
struct ThreadUpperBoundFunctor {
...
void test_upper_bound_thread(const std::vector<T> &_haystack,
  ...
  // get expected value
  const hv_size_type expected = std_upper_bound(_haystack, _needle);
...
template <typename T, typename Device>
void test_upper_bound_team(const std::vector<T> &_haystack, const T _needle) {
  using execution_space = typename Device::execution_space;
...
template <typename T, typename Device>
void test_upper_bound(const std::vector<T> &haystack, const T needle) {
  test_upper_bound_thread<T, Device>(haystack, needle);
  test_upper_bound_team<T, Device>(haystack, needle);
}
...
void test_upper_bound() {
  test_upper_bound<T, Device>({}, T(0));
  test_upper_bound<T, Device>({}, T(1));
  test_upper_bound<T, Device>({}, T(-1));

  test_upper_bound<T, Device>({0}, T(0));
  test_upper_bound<T, Device>({0}, T(1));
  test_upper_bound<T, Device>({0}, T(-1));

  test_upper_bound<T, Device>({1}, T(0));
  test_upper_bound<T, Device>({1}, T(1));
  test_upper_bound<T, Device>({1}, T(-1));

  test_upper_bound<T, Device>({T(-1)}, T(0));
  test_upper_bound<T, Device>({T(-1)}, T(1));
  test_upper_bound<T, Device>({T(-1)}, T(-1));

  test_upper_bound<T, Device>({0, 1, T(2.5), 3, 4, 5}, T(-1));
  test_upper_bound<T, Device>({0, 1, T(2.5), 3, 4, 5}, T(0));
  test_upper_bound<T, Device>({0, 1, T(2.5), 3, 4, 5}, T(1));
  test_upper_bound<T, Device>({0, 1, T(2.5), 3, 4, 5}, T(2));
  test_upper_bound<T, Device>({0, 1, T(2.5), 3, 4, 5}, T(2.4));
  test_upper_bound<T, Device>({0, 1, T(2.5), 3, 4, 5}, T(2.5));
  test_upper_bound<T, Device>({0, 1, T(2.5), 3, 4, 5}, T(2.6));
  test_upper_bound<T, Device>({0, 1, T(2.5), 3, 4, 5}, T(3));
  test_upper_bound<T, Device>({0, 1, T(2.5), 3, 4, 5}, T(4));
  ...
  for (int n = 0; n < numTests; ++n) {
    for (size_t sz : {10, 100, 1000}) {
      ...
      // do the test
      test_upper_bound<T, Device>(haystack, needle);
    }
  }
}
